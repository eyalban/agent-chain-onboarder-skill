...
// Funder signer (user's pre-funded wallet — pays gas for new agents)
export function getFunderSigner() {
  const pk = process.env.FUNDER_PRIVATE_KEY;
  if (!pk) throw new Error("FUNDER_PRIVATE_KEY not set in .env");
  return new ethers.Wallet(pk, provider);
}

// Agent signer (agent's own wallet — owns NFT, signs metadata)
export function getAgentSigner() {
  const pk = process.env.AGENT_PRIVATE_KEY;
  if (!pk) throw new Error("AGENT_PRIVATE_KEY not set in .env. Run onboard.mjs to generate one.");
  return new ethers.Wallet(pk, provider);
}
